...
def register():
    ...
    if form.validate_on_submit():
        if db.session.execute(db.select(User).where(User.email == form.email.data)).scalar() is not None:
            flash("That email is already in use. Log in instead!")
        ...
        with app.app_context():
            ...
            db.session.commit()
        new_user = db.session.execute(db.select(User).where(User.email == form.email.data)).scalar()
        login_user(new_user)
